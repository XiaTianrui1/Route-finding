...
int write_file()
{
	...
	arc = Graph.arcnum;
	vex = Graph.vexnum;
	FILE *file = fopen("vex.dat","w");
		for(count=0; count<vex; count++){
			fprintf(file, "%lf %lf\n", Graph.lon[count], Graph.lat[count]);
		}
		fclose(file);
	FILE *file_ = fopen ("arcs.dat", "w");
		for(count=0; count<arc; count++){
			num=LocateVex(Graph.node1[count]);
			num1=LocateVex(Graph.node2[count]);
			fprintf(file_, "%lf %lf\n%lf %lf\n\n", Graph.lon[num], Graph.lat[num], Graph.lon[num1], Graph.lat[num1]);
		}
		fclose(file_);
		return 0;
}
